...
def md_to_pdf(md_filepath, pdf_filepath):
    ...
    while i < len(lines):
        ...
        # Markdown Image handling: ![alt](path)
        if line.strip().startswith('!['):
            m = re.match(r'^!\[(.*?)\]\((.*?)\)', line.strip())
            if m:
                alt_text = m.group(1)
                img_rel_path = urllib.parse.unquote(m.group(2))
                img_full_path = os.path.normpath(os.path.join(base_dir, img_rel_path))
                if os.path.exists(img_full_path):
                    try:
                        with PILImage.open(img_full_path) as im:
                            w, h = im.size
                            aspect = h / float(w)
                            target_w = 480.0
                            ...
                            rl_img = RLImage(img_full_path, width=target_w, height=target_h)
                            story.append(Spacer(1, 4))
                            story.append(rl_img)
                            if alt_text:
                                story.append(Paragraph(f"Visual Exhibit: {alt_text}", caption_style))
                            story.append(Spacer(1, 6))
                    except Exception as e:
                        print(f"Error embedding image {img_full_path}: {e}")
                i += 1
                continue
